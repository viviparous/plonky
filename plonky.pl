...
my %gdDbg=(level=>0 ,msg=>"DBGmode", exp=>"expected");
...
sub doMsgr { my $arf=shift; my $sep='x' x 60; say $sep if $arf->[0]==1; say join(" ", @$arf); say $sep if $arf->[0]==1; }
...
my $sCpuImplm=qx{cat /proc/cpuinfo | grep "CPU implementer" | wc -l}; chomp($sCpuImplm); #an ARM property
...
my $owtype= qq/own_window_type = 'normal',/; 
if( $dARCHVals{wmtype} == 1){ $owtype = qq/own_window_type = 'desktop',/; } 
push @aCFG, $owtype;
...
if( $sCpuNym =~ $aCPUTypes[4] ) {  #bIsRPi , show temperature

 push @aTXT, "\${execi 5 /usr/bin/vcgencmd measure_temp }";
 push @aTXT, "\$stippled_hr";

}

push @aTXT, "\${color black}\${cpugraph 5000a0 f000a0 }";

my $ih=25;
$ih = 13 if($iCpuCount > 5); 
